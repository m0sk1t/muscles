angular.module('Services', []).factory('User', ['$http',
	function($http) {
		return {
			...
			forgot: function(credentials, s, e) {
				return $http.post('/forgot', credentials).then(s, e);
			},
			reset: function(credentials, s, e) {
				return $http.post('/reset/' + credentials.token, credentials).then(s, e);
			},
